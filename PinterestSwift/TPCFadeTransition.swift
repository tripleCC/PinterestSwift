...
class TPCCollectionToPageZoomTransition: TPCTransitionProtocol {
    ...
    func isSatisfyRequirementByFromVc(fromVc: UIViewController, toVc: UIViewController) -> Bool {
    ...
    func popTransitionFromVc(fromVc: UIViewController, toVc: UIViewController, containerView: UIView, completion: (() -> Void)) {
        ...
        let offsetY : CGFloat = fromVc.navigationController!.navigationBarHidden ? 0.0 : navigationHeaderAndStatusbarHeight
        snapShot.origin(CGPointMake(0, -pullOffsetY+offsetY))
        containerView.addSubview(snapShot)
//        (toVc as! NTWaterFallViewControllerProtocol).viewWillAppearWithPageIndex(indexPath.row)
        toView.hidden = false
        toView.alpha = 0
